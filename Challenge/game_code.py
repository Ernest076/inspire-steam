...
#See the problems beow, try to solve them
#Ensure you can at least solve the first problem

# PROBLEM 1
#When a charcter kicks anothe character, the opponent gets more damage than what the character deals
#Once this problem is solved, the final health of the warrior after taking damage should be 120

# PROBLEM 2
#The function to tackle other opponents is not defined
#Solve the method, then let the warrior character tackle the ninja charactr
...
class FighterCharacter:
    ...
    def run(self, direction):
        print(f"Game log:{self.character_role} runs {direction} at speed {self.character_speeed}")

    def report_status(self):
        print(f"\nCharacter Log: \nRole:{self.character_role} \nHealth:{self.character_health} \nDamage:{self.character_damage} \nSpeed:{self.character_speed}|n __ \n")
    ...
    def tackle(self, opponent):
        # Implement his so that the opponents character_speed is reduced by the damage dealt by the fighter
        # For instance if the ninja's speed is 120, a tackle from the warrior should reduce their speed to 80
        # Remember to remove the pass below before running your trail pass
